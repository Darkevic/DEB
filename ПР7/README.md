...
DAC — это дискреционный контроль: владелец ресурса сам решает, кому дать доступ. MAC — мандатный контроль: правила задаёт система, а не пользователь. Одного DAC недостаточно, потому что владелец может случайно или намеренно выдать лишние права.

**2. Что означает запись cap_net_bind_service=eip? Чем ep отличается от eip?**  
`cap_net_bind_service=eip` значит, что capability разрешает процессу bind к привилегированным портам и сохраняется в эффективном, наследуемом и разрешённом наборах. `ep` — это effective + permitted, а `eip` добавляет ещё inheritable.

**3. В чём разница между complain и enforce в AppArmor? Зачем нужен complain?**  
`enforce` блокирует запрещённые действия, а `complain` только логирует их, не мешая работе. `complain` нужен для отладки и настройки профиля перед включением жёсткого режима. 

**4. Docker использует то же ядро что и хост — почему тогда контейнер изолирован?**  
Изоляция достигается не отдельным ядром, а механизмами namespaces, cgroups, capabilities и профилями вроде AppArmor/SELinux. Поэтому контейнер видит только свою “область” процессов, сети и файловых ресурсов.
...
Он может выполнить код внутри контейнера с правами того пользователя и использовать доступные ресурсы контейнера. Но ему сильно ограничены root-права, возможности работы с хостом, привилегированные операции и то, что запрещает AppArmor/capabilities. 

**6. Почему --privileged — антипаттерн? Когда всё же оправдан?**  
`--privileged` почти снимает основные ограничения контейнера и резко расширяет его доступ к хосту, поэтому это опасно. Оправдан он только в узких тестовых или очень специальных средах, где без полного доступа не обойтись. 
